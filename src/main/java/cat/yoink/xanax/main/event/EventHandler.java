...
public final class EventHandler implements MinecraftInstance
{
    ...
    {
        Manager.EVENT_BUS.dispatchEvent(new KeyboardEvent(Keyboard.getEventKey(), Keyboard.getEventKeyState()));
    ...
    {
        if (!nullCheck()) Manager.EVENT_BUS.dispatchEvent(new TickEvent());
    }

    @SubscribeEvent
    public void onAttackEntity(net.minecraftforge.event.entity.player.AttackEntityEvent event)
    {
        if (!nullCheck() && Manager.EVENT_BUS.dispatchEvent(new AttackEntityEvent(event.getEntityPlayer())).isCancelled())
    ...
    {
        if (!nullCheck() && Manager.EVENT_BUS.dispatchEvent(new BlockPushEvent()).isCancelled())
            event.setCanceled(true);
    }
